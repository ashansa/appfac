...
public class DeployNotifier {
    ...
    public void deployed(String applicationId, String version, String stage, String userName, String password, String endpoint,
        ...
        if (debugEnabled) {
            log.debug("Observer is called to updated deployed information user name = " + userName + "tenant domain =" + tenantDomain);
        }

        StringBuilder builder = new StringBuilder();
        builder.append(userName).append('@').append(tenantDomain);
        try {
            ...
            //adding mutual auth header to load tenant in the calling node
            String mutualAuthHeader = "<tns:UserName xmlns:tns=\"http://mutualssl.carbon.wso2.org\">" + builder.toString() + "</tns:UserName> ";
            client.addHeader(AXIOMUtil.stringToOM(mutualAuthHeader));
            if (debugEnabled) {
                log.debug("mutual auth header => \n " + mutualAuthHeader);
            }
            ...
            String msg = "Problem when setting mutual ssl header for updateDeploymentInformation service";
            log.error(msg, e);
        }
    }
